...
class GUI_Comment extends GUI {
    ...
    public function __construct($data) {
        $this->html = file_get_contents("template/commentListItem.html");
        parent::__construct($data);
    }
    ...
    public function buildHTML() {
        ...
        foreach ($this->data->children as $comment) {
            ...
            $commentChildren = $commentChildren . $gui->getHTML();
        }

        $this->html = str_replace("[DATA:COMMENT_ID]", $commentId, $this->html);
        $this->html = str_replace("[DATA:USER_ID]", $userId, $this->html);
        ...
        $this->html = str_replace("[DATA:COMMENT_CHILDREN]", $commentChildren, $this->html);
    }
...
class GUI_CommentList extends GUI {
    ...
    public function __construct($data) {
        $this->html = file_get_contents("template/commentListWrapper.html");
        parent::__construct($data);
    }
    ...
    public function buildHTML() {
        ...
        foreach ($this->data as $comment) {
            $gui = new GUI_Comment($comment);
            $commentListContent = $commentListContent . $gui->getHTML();
        }
        $this->html = str_replace("[DATA:COMMENT_LIST_CONTENT]", $commentListContent, $this->html);
    }
